(function () {
	...
	Colorpicker.prototype = {
		...
		init(opt) {
			...
			this.elem_cancel=document.getElementById("color-cancel");
			this.elem_confirm=document.getElementById("color-confirm");
			...
			const clientWidth =  this.elem_colorPancel.clientWidth + this.elem_colorPalette.clientWidth;
			const clientHeight = this.elem_colorPancel.clientHeight;
			...
			this.pancelLeft =( window.innerWidth / 2) - (clientWidth/2) + this.elem_colorPalette.clientWidth;
			this.pancelTop = (window.innerHeight/2 -clientHeight  );
			window.onresize = () => {
				this.pancelLeft =( window.innerWidth / 2) - (clientWidth/2) + this.elem_colorPalette.clientWidth;
				this.pancelTop = (window.innerHeight/2 -clientHeight  );
			};
			util.css(div, {
				...
				"left": "50%",
				"top": "50%",
				"transform":`translate(-${clientWidth/2}px,-${clientHeight}px)`
			});
			...
			this.elem_cancel.addEventListener("click",function()  {
				_this.cancel();
			});
			this.elem_confirm.addEventListener("click",function(){
				_this.Opt.confirm('#'+_this.rgbToHex(_this.HSBToRGB(_this.hsb)));
				_this.hide();
			});
		...
		render: function () {
			const tpl =
				...
				<div style="position: inherit;z-index: 100;display: flex;box-shadow: rgba(0, 0, 0, 0.3) 0px 0px 2px, rgba(0, 0, 0, 0.3) 0px 4px 8px;">
					<div style='width:128px;padding:5px 10px;background: #f9f9f9;' class='color-palette'>
						<div>标准颜色</div>
						<div style='display:flex;flex-flow: row wrap;align-content: space-around;justify-content: space-around;height: 250px;' >
					...
					<div class="colorpicker-pancel" style="background: rgb(255, 255, 255);box-sizing: initial; width: 225px; font-family: Menlo;">
						<div style="margin: 5px 3px;">自定义颜色</div>
						...
						<div class="color-btn-row">
							<button class="color-button" id="color-confirm">确定</button>
							<button class="color-button" id="color-cancel">取消</button>
						</div>
					</div>
				</div>
			</div>
		...
		getPaletteColorsItem: function () {
			let str = '';
			let palette =  [ 
				'#0072bd', '#d95319', '#ede120', '#7e2f8e', '#77ac30', '#4dbeee', '#a2142f',
				'#ffff11','#19a5ff', '#ff6929', '#64d413', '#b646ff', '#0fffb5', '#ff19a6',
				'#ffffff', '#f0f0f0', '#e6e6e6', '#cccccc', '#a6a6a6',  '#808080','#262626', 
				'#ff0000', '#ff00ff', '#ffff00', '#00ff00', '#00ffff', '#0000ff', '#000000' 
			  ];
		...
		setPosition(x, y) {
			var LEFT = parseInt(x - this.pancelLeft),
				TOP = parseInt(y - this.pancelTop - 16);
			...
			switch (this.current_mode) {
				...
				case 'rgb':
					...
					for (let index = 0; index < inputs.length; index++) {
						...
						if(element.value && typeof parseInt(element.value) === 'number'){
