...
export default {
    ...
    computed: {
        ...mapState(rootStore.useDataCenterStore, ['curComponent', 'editor', 'canvasStyleData']),
        ...mapState(rootStore.useComposeStore, ['editor', 'canvasStyleData']),
    ...
    methods: {
        ...
        handleRotate(e) {
            ...
            const move = (moveEvent) => {
                ...
                // 修改当前组件样式
                rootStore.dataCenter.setShapeStyle(pos)
            };

            const up = () => {
                hasMove && rootStore.snapshot.recordSnapshot()
        ...
        getPointStyle(point) {
            ...
            if (width.toString().includes('%')) {
                realWidth = (Number(this.canvasStyleData.width) * parseFloat(width)) / 100;
            }
        ...
        handleMouseDownOnShape(e) {
            ...
            rootStore.editor.setClickComponentStatus(true)
            const element = document.getElementById(`component${this.element.id}`);
            if (isPreventDrop(this.element.component)) {
                e.preventDefault();
            }
            if (!['Tabs', 'GridLayout', 'ErLayout', 'ErGrid', "ErTabs", 'VPanel', 'ErCollapse'].includes(this.element.component)) {
                if (this.element.pid) {
            ...
            const move = (moveEvent) => {
                ...
                this.$nextTick(() => {
                    ...
                    eventBus.$emit('move', curY - startY > 0, curX - startX > 0, this.element);
                });
            };
        ...
        handleMouseDownOnPoint(point, e) {
            ...
            // 获取画布位移信息
            let editorRectInfo;
            if (this.curComponent.pid) {
                editorRectInfo = document.getElementById(this.curComponent.activeName).getBoundingClientRect();
            } else {
                editorRectInfo = this.editor.getBoundingClientRect();
            }
            ...
            const move = (moveEvent) => {
                ...
                rootStore.dataCenter.setShapeStyle(style)
            };

            const up = () => {
                document.removeEventListener('mousemove', move);
                document.removeEventListener('mouseup', up);
                needSave && rootStore.snapshot.recordSnapshot()
            };
