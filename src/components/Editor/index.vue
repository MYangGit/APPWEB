<template>
    ...
    >
        ...
        <!--页面组件列表展示-->
        <Shape
            v-for="(item, index) in componentData.filter((i) => !i.pid)"
            :key="item.id"
...
import { $, isPreventDrop, createUuid } from '@/utils/utils';
...
export default {
    ...
    methods: {
        ...
        handleContextMenu(e) {
            ...
            let target = e.target;
            let top = e.pageY - 100;
            const pageX = body[0].clientWidth - 528 - this.canvasStyleData.width;
            let left = e.pageX - 220 - (pageX > 0 ? pageX : 0) / 2;
